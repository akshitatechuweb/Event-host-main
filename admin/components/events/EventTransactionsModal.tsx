...
export default function EventTransactionsModal({
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-[95vw] w-full sm:max-w-6xl max-h-[90vh] p-0 gap-0 rounded-3xl border-0 bg-background/95 backdrop-blur-xl shadow-2xl">
        
        {/* Header */}
        <DialogHeader className="sticky top-0 z-10 px-8 py-6 border-b border-border/50 bg-background/80 backdrop-blur-sm rounded-t-3xl">
        ...
        <div className="overflow-y-auto px-8 py-6 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <StatCard label="Total Revenue" value={`₹${totals.totalRevenue.toLocaleString()}`} />
            <StatCard label="Total Transactions" value={totals.totalTransactions} />
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border/50 bg-muted/20">
                    {["Date", "Buyer", "Order ID", "Items", "Amount", "Status"].map((h) => (
                      <th
                        key={h}
