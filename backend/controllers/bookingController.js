...
export const createBooking = async (req, res) => {
  try {
    const { eventId, selectedTickets = [], attendees = [], pricePaid } = req.body;
    ...
    if (!Array.isArray(selectedTickets) || selectedTickets.length === 0) {
      return res.status(400).json({ success: false, message: "selectedTickets must be provided" });
    }

    if (!Array.isArray(attendees) || attendees.length === 0) {
      return res.status(400).json({ success: false, message: "attendees must be provided" });
    }
    ...
    for (const item of selectedTickets) {
      ...
      if (!pass) return res.status(400).json({ success: false, message: `${item.type} pass not available` });
      if (pass.remainingQuantity < item.quantity) return res.status(400).json({ success: false, message: `${item.type} pass insufficient stock` });
      totalAmount += pass.price * item.quantity;
      totalPersonsRequired += item.type === "Couple" ? item.quantity * 2 : item.quantity;
    }
    ...
    for (const att of attendees) {
      const ticketNumber =
        "TKT-" + Date.now() + "-" + Math.random().toString(36).substring(2, 10).toUpperCase();
      const qrData = JSON.stringify({ ticketNumber, bookingId: booking._id, eventId, attendeeName: att.fullName, ticketType: att.passType });
      const pass = passMap[att.passType];
      const price = pass ? (pass.type === 'Couple' ? (pass.price / 2) : pass.price) : 0;
      const newTicket = await GeneratedTicket.create({ bookingId: booking._id, eventId, userId, ticketNumber, qrCode: qrData, attendee: att, ticketType: att.passType, price });
...
export const getMyBookings = async (req, res) => {
  try {
    ...
    const tickets = await GeneratedTicket.find({ bookingId: { $in: bookingIds } });
